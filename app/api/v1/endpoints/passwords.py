...
@router.get(
    "/{password_id}", response_model=PasswordResponse, status_code=status.HTTP_200_OK
)
def get_password_endpoint(
    ...
    user_id = current_user.id
    password = get_password_by_id(db, user_id, password_id)
    if not password:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail="Password not found or unauthorized",
        )
    return password
